<template>
  <div class="map-container">
    <div class="header">
      <h2>📍 작전 구역: 정동길의 비밀</h2>
      <p>목표 지점에 도달하여 단서를 수집하라. (현재 힌트: {{ collectedHints }} / {{ requiredHints }})</p>
    </div>
...
const initMap = (missionData) => {
  ...
  missionData.forEach(m => {
    // 핵심: 최종 목적지인데 힌트가 부족하면 마커를 찍지 않음
    if (m.isFinal && collectedHints.value < requiredHints.value) {
      return;
    }
...
const uploadImage = async (event) => {
  ...
  try {
    ...
    router.push(`/chat/${currentSessionId.value}`); // 분석 성공시 채팅뷰로 넘어감
  } catch (error) {
    alert("인증 실패: 유효한 단서를 찾을 수 없습니다.");
  }
};

// TODO: [테스트용] 도착 & 힌트 획득 버튼
const forceArrival = async () => {
  try {
    ...
    if(collectedHints.value >= requiredHints.value) {
      ...
      initMap(missions.value); // 지도 다시 그리기
    }
  } catch (error) {
...
.map-container { padding: 20px; background-color: #1a1a1a; min-height: 100vh; color: #00ffcc; font-family: 'Share Tech Mono', monospace; }
.header { text-align: center; margin-bottom: 20px; }
.game-ui { margin-top: 20px; padding: 20px; background-color: #050505; border-radius: 12px; border: 1px solid #333; }
.btn { padding: 12px 20px; border-radius: 8px; border: none; font-weight: bold; cursor: pointer; transition: 0.3s; font-family: 'Share Tech Mono', monospace; }
.debug-btn { background-color: #440000; color: #ff4444; border: 1px solid #ff4444; width: 100%; margin-bottom: 15px; text-transform: uppercase; }
