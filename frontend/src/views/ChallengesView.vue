  ...
  <main class="challenges-page">
    ...
    <section v-else class="challenge-grid">
      <article class="panel">
        ...
        <div v-for="challenge in challenges" :key="challenge.id" class="challenge-card" :class="{ completed: challenge.completed }">
          <div class="card-top">
          ...
          <button v-else type="button" class="ghost" @click="router.push({ name: 'Ranking' })">기록 보기</button>
        </div>
      </article>
      ...
      <article class="panel">
        <div class="panel-head">
          ...
          <span>{{ myChallenges.length }}개</span>
        </div>
        <p v-if="!myChallenges.length" class="empty">아직 참가한 챌린지가 없습니다.</p>
        <div v-for="challenge in myChallenges" :key="`mine-${challenge.id}`" class="mini-card">
          <strong>{{ challenge.title }}</strong>
...
<style scoped>
.challenges-page { min-height: 100vh; box-sizing: border-box; padding: 26px 16px 72px; background: radial-gradient(circle at 75% 0%, rgba(248,113,113,.22), transparent 32%), linear-gradient(150deg, #1f1111, #111827 58%, #030712); color: #f8fafc; font-family: 'Noto Serif KR', Georgia, serif; }
