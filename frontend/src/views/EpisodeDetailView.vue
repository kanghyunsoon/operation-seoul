<template>
  <main class="detail-page">
    <button class="back" type="button" @click="router.push({ name: 'EpisodeList' })">목록</button>
...
.panel { width: min(100%, 720px); margin: 0 auto; padding: 24px; border-radius: 22px; background: linear-gradient(155deg, rgba(120,53,15,.28), rgba(15,23,42,.9)); border: 1px solid rgba(245,158,11,.22); }
.eyebrow { color: #f59e0b; font-weight: 900; letter-spacing: .08em; } h1 { margin: 0; font-size: clamp(1.8rem, 9vw, 3.4rem); line-height: 1.05; } h2 { color: #fde68a; font-size: 1.05rem; } .synopsis { color: #dbeafe; line-height: 1.75; white-space: pre-line; }
.info-grid { display: grid; gap: 8px; margin: 20px 0; } .info-grid span { display: flex; justify-content: space-between; gap: 10px; padding: 12px; border-radius: 12px; background: rgba(2,6,23,.34); color: #94a3b8; } strong { color: #fff; }
.primary { width: 100%; min-height: 48px; border: 0; border-radius: 14px; background: #b45309; color: #fff; font-weight: 900; font: inherit; } .message { color: #bbf7d0; }
</style>