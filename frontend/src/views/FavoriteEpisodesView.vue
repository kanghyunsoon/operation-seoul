<template>
  <main class="tab-page">
    ...
    <section v-if="loading" class="state">관심목록을 불러오는 중입니다.</section>
    <section v-else-if="error" class="state error">{{ error }}</section>
    <section v-else-if="!favorites.length" class="state">아직 관심목록에 담긴 미션이 없습니다.</section>
    <section v-else class="card-grid">
      <article v-for="item in favorites" :key="item.favoriteId || item.episodeId" class="case-card">
        <div class="card-head">
          <span>CASE {{ String(item.episodeId).padStart(2, '0') }}</span>
          <button type="button" :disabled="busyId === item.episodeId" @click="removeFavorite(item)">해제</button>
        </div>
        <h2>{{ item.title }}</h2>
        <p>{{ item.subtitle }}</p>
        <div class="meta">
...
async function loadFavorites() {
  ...
  try {
    ...
    error.value = err.userMessage || '관심목록을 불러오지 못했습니다.';
  } finally {
    loading.value = false;
  }
}
...
async function removeFavorite(item) {
  ...
  try {
    ...
    setMessage('관심목록에서 제거했습니다.');
  } catch (err) {
    setMessage(err.userMessage || '관심목록에서 제거하지 못했습니다.', 'error');
  } finally {
    busyId.value = null;
  }
}
...
<style scoped>
.tab-page { min-height: 100vh; box-sizing: border-box; padding: 24px 16px 126px; background: radial-gradient(circle at 80% 0%, rgba(245,158,11,.2), transparent 34%), linear-gradient(160deg, #0f172a, #111827 60%, #050505); color: #f8fafc; font-family: Georgia, 'Noto Sans KR', serif; }
.hero, .card-grid, .state, .toast { width: min(100%, 880px); box-sizing: border-box; margin-left: auto; margin-right: auto; }
...
.case-card { padding: 20px; border-radius: 18px; border: 1px solid rgba(248,250,252,.13); background: linear-gradient(135deg, rgba(255,247,237,.09), rgba(15,23,42,.78)); box-shadow: 0 20px 52px rgba(0,0,0,.22); }
.card-head { display: flex; align-items: center; justify-content: space-between; gap: 12px; }
.card-head span { color: #fca5a5; font-size: .72rem; font-weight: 900; letter-spacing: .1em; }
h2 { margin: 14px 0 8px; font-size: 1.28rem; }
