<template>
  <main class="ranking-page">
    <header class="hero">
      <button type="button" class="ghost" @click="router.push({ name: 'EpisodeList' })">미션 파일 보기</button>
      <p>LEADERBOARD</p>
      <h1>{{ rankingTitle }}</h1>
      <span>이 미션의 점수, 오답 수, 추리 질문 수, 제출 횟수, 클리어 시간을 기준으로 정렬합니다.</span>
    ...
    <section class="board-shell">
      ...
      <article class="panel player-board">
        ...
        <div v-if="!playerRankings.length" class="empty">아직 합산 랭킹 기록이 없습니다.</div>
        <ol v-else class="ranking-list player-list">
          <li v-for="entry in playerRankings" :key="`player-${entry.userId}`">
            <b>{{ entry.rankNo }}</b>
            <img :src="entry.profileImageUrl || defaultProfile" alt="" />
            <div>
              <strong>{{ entry.nickname || '요원' }}</strong>
...
const message = ref('');
const episodeId = computed(() => route.query.episodeId ? Number(route.query.episodeId) : null);
const rankingTitle = computed(() => route.query.title ? `${route.query.title} 랭킹` : '미션 랭킹');
...
watch(() => route.query.episodeId, loadRankings);
...
async function loadRankings() {
  ...
  try {
    ...
    const [rankingData, myData, playerData] = await Promise.all([
      rankingApi.getRankings(params),
      rankingApi.getMyRankings(episodeId.value ? { episodeId: episodeId.value } : {}),
      rankingApi.getPlayerRankings({ limit: 50 })
    ]);
    rankings.value = rankingData;
    myRankings.value = myData;
    playerRankings.value = playerData;
  } catch (error) {
