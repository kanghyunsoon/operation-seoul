...
- 내부 `DESTINATION_HINT` 역할은 사용자에게 “목적지 힌트”로 표시하지 않고, 장소 키워드 미션 그룹으로만 사용합니다.
- 장소 키워드 미션이 모두 완료되기 전에는 실제 최종 장소를 지도 응답에서 제외합니다.
- 장소 키워드 미션이 모두 완료되면 실제 최종 장소를 `FINAL_DESTINATION` 공개 마커로 내려보내며, 프론트는 검은색 마커로 표시합니다.
...
- 사용자는 `publicMarkerType`만 보고 지도를 봅니다. 일반 미션은 `KEYWORD_1`, `KEYWORD_2`, `KEYWORD_3` 색상으로 구분되고, 최종 목적지는 공개 조건 충족 후 `FINAL_DESTINATION`으로만 나타납니다.
...
정답 키워드는 사용자가 단서 카드에서 하나씩 확인하거나 추론할 수 있는 짧은 원자 키워드여야 합니다. `유달산의 잊혀진 항로`처럼 장소, 수식어, 대상이 한 문장처럼 붙은 복합 키워드는 사용자가 맞히기 어렵기 때문에 피합니다.
...
최종 장소는 내부 판정용입니다. 플레이 중에는 목적지 힌트라는 별도 분류를 사용자에게 보여주지 않고, 세 가지 키워드 색상의 미션만 표시합니다. 장소 키워드 미션이 모두 해결되면 실제 최종 목적지가 검은색 마커로 지도에 나타납니다.
...
- 장소 좌표가 실제 위치와 맞는가
- 도착 반경이 현장에서 무리 없는가
- 장소 키워드 미션 완료 전에는 최종 장소가 지도 응답에서 숨겨지고, 완료 후에는 검은 최종 목적지 마커로만 공개되는가
