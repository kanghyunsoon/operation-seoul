...
1. 모티브 공개
2. 실제 사건 해설
3. 픽션과 역사/문화 모티브의 매칭, 디브리핑

Fact Mode에서는 최종 정답 입력 장소의 역사/문화 모티브를 명확히 설명할 수 있습니다. 단, 허구 사건이 실제 장소에서 실제로 벌어진 일처럼 보이면 안 됩니다.

## 10. 최종 장소 공개 규칙

이 프로젝트에서 최종 장소는 추리 정답이 아니라 최종 정답 입력 장소입니다. 플레이어는 장소를 맞히는 것이 아니라, 조사 단서로 `범인`, `흉기`, `동기`, `방법`을 도출합니다.

서버 내부:

- `mission_spots.is_final_place` 또는 동등한 내부 필드로 실제 최종 장소를 판정합니다.
- 최종 장소는 TourAPI 장소를 사용하며, 허구 사건의 배경 모티브로만 활용합니다.
- 시작 장소와 최종 장소를 제외한 조사 미션 8개가 모두 완료되기 전에는 최종 장소를 지도 응답에서 제외합니다.
- 조사 미션 8개가 모두 완료되면 최종 장소를 `FINAL` 공개 마커로 내려보냅니다.
- 최종 장소 도착 시 `FINAL_READY` 상태가 됩니다.
- 최종 추리와 최종 정답 제출은 이 상태와 연결됩니다.

사용자 응답:

- 일반 사용자 지도 응답에는 최종 장소 여부를 나타내는 `finalPlace` 플래그가 포함될 수 있지만, 최종 장소는 잠금 해제 전까지 응답에서 제외됩니다.
- 사용자는 `publicMarkerType`을 보고 지도를 봅니다. 일반 미션은 조사 미션으로 표시되고, 최종 장소는 공개 조건 충족 후 `FINAL`로만 나타납니다.
...
- 보상 단서는 `범인`, `흉기`, `동기`, `방법` 중 하나 이상의 추리를 진전시켜야 합니다.
...
`finalAnswerKeywords`는 정답 판정에 필요한 필수 키워드 목록입니다. 현재 AI 생성 에피소드는 항상 `범인`, `흉기`, `동기`, `방법` 4개 슬롯을 사용합니다. `finalAnswer`는 이 키워드들을 자연어 문장으로 묶은 최종 진실입니다.

정답 키워드는 사용자가 단서 카드에서 하나씩 확인하거나 추론할 수 있는 짧은 원자 키워드여야 합니다. 장소명이나 목적지 추론 키워드를 정답 슬롯으로 쓰면 안 됩니다.

예시:

```text
finalAnswerKeywords = ["강수진", "독이 섞인 수면제 캡슐", "해고 통보에 대한 복수", "피해자의 매일 복용 약을 독성 캡슐로 바꿔치기"]
finalAnswer = "강수진이 해고 통보에 대한 복수로 피해자의 매일 복용 약을 독이 섞인 수면제 캡슐로 바꿔치기했다."
```

사용자 답안에는 4개 슬롯의 의미가 모두 포함되어야 정답입니다.
...
- `TourAPI 장소명`
- `실제 역사 사건명`
- `정동의 그림자`처럼 슬롯이 불명확한 문장형 키워드

픽션 시놉시스와 최종 질문은 정답 키워드를 직접 노출하지 않아야 하지만, `가려진 4자 단서`, `9자 단서`처럼 글자 수만 보여주는 표현도 쓰면 안 됩니다. 대신 “기록되지 않은 항로”, “방해자의 정체”, “봉인된 물건의 행방”, “마지막 암호의 의미”처럼 스토리 역할을 설명해야 합니다.

### Fiction Mode와 실제 역사

플레이 중에는 실제 역사/문화 해설을 숨기고, 클리어 후에만 배경 모티브와 픽션의 대응 관계를 보여줍니다. 허구 범죄가 실제 장소에서 일어난 것처럼 쓰면 안 됩니다.

### 최종 장소와 조사 단서

최종 장소는 내부 판정용이자 최종 정답 입력 장소입니다. 장소 힌트나 목적지 추리 시스템을 만들지 않습니다. 시작 장소를 제외한 조사 미션 8개가 모두 해결되면 최종 장소가 `FINAL` 마커로 지도에 나타납니다.
...
`deductionForbiddenReveals`는 최종 추리 중 직접 말하면 안 되는 금지어 목록입니다. 최종 정답과 직접 스포일러가 들어갑니다. 최종 장소는 정답 키워드가 아니므로 장소 추리용 금지어로 설계하지 않습니다.
...
- 장소 좌표가 실제 위치와 맞는가
- 도착 반경이 현장에서 무리 없는가
- 조사 미션 8개 완료 전에는 최종 장소가 지도 응답에서 숨겨지고, 완료 후에는 `FINAL` 마커로만 공개되는가
- 퍼즐이 실제 제공 데이터 또는 관리자 메모에 근거하는가
- 픽션 시놉시스와 최종 질문과 최종 정답이 `범인`, `흉기`, `동기`, `방법` 4개 슬롯을 같은 목표로 가리키는가
