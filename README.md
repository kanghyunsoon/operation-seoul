...
| --- | --- | --- | --- |
| 팀원 1 / 팀장 | 이름 기입 | 게이미피케이션 구조, AI 프롬프트, DB 모델, 보안/인증, 발표 산출물 총괄 | 리뷰/랭킹/팔로우 정책 확정, MyBatis 전환 여부 판단, 발표자료 완성 |
| 팀원 2 / 지도·로케이션 | 이름 기입 | Kakao 지도, GPS 도착 판정, 지역 선택, 주변 POI, 이동 동선, DB 연계 보조 | 무장애/두루누비 기반 코스 필터링, 현장 QA, 지도 화면 안정화 |
